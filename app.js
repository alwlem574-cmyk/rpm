/* نظام حسن الوليم RPM - V5 (Cloud only + Activity Log)
   - زباين + سيارات + سجل
...
async function writeActivity({ action, store, entityId = "", before = null, after = null, note = "" } = {}) {
  try {
    ...
    const entry = {
      ...
      before: _truncateForLog(before),
      after: _truncateForLog(after),
      note: String(note || ""),
    };
...
const ROLE_ROUTES = {
  admin: ["*","activity"],
  accountant: ["dashboard","orders","order","customers","customer","vehicles","vehicle","invoices","reports","reportfull","expenses","backup","dedupe","custom","more","auth","activity"],
  reception: ["dashboard","checkin","appointments","orders","order","customers","customer","vehicles","vehicle","invoices","custom","more","auth","activity"],
  technician: ["dashboard","appointments","orders","order","customers","customer","vehicles","vehicle","custom","more","auth","activity"],
  pending: ["dashboard","more","auth"],
};

// صلاحيات تفصيلية (غير مسموح للواجهة فقط — تُستخدم أيضاً لحماية الأزرار/الإجراءات)
const ROLE_PERMS = {
  admin: {
    all: true,
    ...
    delete_master: true,       // حذف زباين/سيارات/أوامر/قطع… إلخ
    custom_admin: true,
    roles_manage: true,
  },
  accountant: {
...
function hasPerm(perm) {
  const r = currentRole();
  const p = ROLE_PERMS[r] || ROLE_PERMS.pending;
...
async function viewRoles() {
  ...
  return `
    <div class="card">
      ...
      <div class="card subcard">
        <div style="font-weight:900">مصفوفة الصلاحيات</div>
        <div class="small">ملخص سريع: ماذا يستطيع كل دور؟</div>
        <div class="hr"></div>
        <div class="grid2">
          <div class="card subcard"><div style="font-weight:900">مدير</div><div class="small">كل شيء + حذف + صلاحيات + مخزون + مصروفات + دفعات</div></div>
          <div class="card subcard"><div style="font-weight:900">محاسب</div><div class="small">فواتير (إنشاء/دفعات) + مصروفات + مخزون (تعديل) بدون حذف حساس</div></div>
          <div class="card subcard"><div style="font-weight:900">استقبال</div><div class="small">زباين/سيارات/مواعيد/أوامر + إنشاء فاتورة (بدون دفعات/حذف)</div></div>
          <div class="card subcard"><div style="font-weight:900">فني</div><div class="small">أوامر شغل + تحديث حالات + بدون أسعار/مخزون/حذف</div></div>
        </div>
      </div>
              ...
              <tr class="tr">
                ...
                <td>
                  <div style="display:flex;gap:8px;flex-wrap:wrap">
                    ...
                    <button class="btn" data-act="makeInvoice" data-id="${w.id}">فاتورة</button>
                    ${hasPerm("delete_master") ? `<button class="btn btn-danger" data-act="deleteWO" data-id="${w.id}">حذف</button>` : ``}
                  </div>
                </td>
              </tr>
            `;
            ...
            <tr class="tr">
              ...
              <td>
                <div style="display:flex;gap:8px;flex-wrap:wrap">
                  ...
                  ${canDelCust ? `<button class="btn btn-danger" data-act="deleteCustomer" data-id="${c.id}">حذف</button>` : ``}
                </div>
              </td>
            </tr>
          `).join("")}
              ...
              <tr class="tr">
                ...
                <td>
                  <div style="display:flex;gap:8px;flex-wrap:wrap">
                    ...
                    ${hasPerm("delete_master") ? `<button class="btn btn-danger" data-act="deleteVehicle" data-id="${v.id}">حذف</button>` : ``}
                  </div>
                </td>
              </tr>
            `;
...
async function viewInventory() {
  const canInv = hasPerm('inventory_manage');
  const parts = (await dbAPI.getAll("parts")).sort((a,b)=> (a.name||"").localeCompare(b.name||"", "ar"));
  ...
  return `
    <div class="card">
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap">
        ...
        ${canInv ? `<button class="btn btn-primary" data-act="newPart">+ قطعة</button>` : ``}
      </div>
      ...
      <div class="grid2">
        ...
        <div style="display:flex;gap:8px;align-items:end;flex-wrap:wrap">
          ...
          ${canInv ? `<button class="btn btn-primary" data-act="excelImportParts">استيراد Excel</button>` : ``}
        </div>
      </div>
        ...
        <tbody>
          ${parts.map(p => `
            <tr class="tr">
              ...
              <td>
                ${canInv ? `<div style="display:flex;gap:8px;flex-wrap:wrap">
                ...
                </div>` : `<span class="small">قراءة فقط</span>`}
              </td>
            </tr>
          `).join("")}
        </tbody>
...
async function viewInvoices() {
  const canPay = hasPerm('invoices_pay');
  const canDelInv = hasPerm('invoices_delete');
  const invoices = (await dbAPI.getAll("invoices")).sort((a,b)=>b.createdAt-a.createdAt);
  const workOrders = await dbAPI.getAll("workOrders");
  const customers = await dbAPI.getAll("customers");
  const vehicles = await dbAPI.getAll("vehicles");
          ...
          ${invoices.map(inv => {
            ...
            return `
              <tr class="tr">
                ...
                <td>
                  <div style="display:flex;gap:8px;flex-wrap:wrap">
                    ${canPay ? `<button class="btn" data-act="invoicePay" data-id="${inv.id}">دفعة</button>` : ``}
                    ...
                    ${canDelInv ? `<button class="btn btn-danger" data-act="deleteInvoice" data-id="${inv.id}">حذف</button>` : ``}
                  </div>
                </td>
              </tr>
            `;
...
// RBAC: حماية الإجراءات الحساسة (مو بس إخفاء أزرار)
const ACT_PERMS = {
  // حذف/تعديل حساس
  deleteCustomer: "delete_master",
  deleteVehicle: "delete_master",
  deleteWO: "delete_master",
  deletePart: "inventory_manage",
  stockAdd: "inventory_manage",
  stockSub: "inventory_manage",
  newPart: "inventory_manage",
  excelImportParts: "inventory_manage",

  deleteInvoice: "invoices_delete",
  invoicePay: "invoices_pay",
  makeInvoice: "invoices_create",

  delExpense: "expenses_manage",
  newExpense: "expenses_manage",
  editExpense: "expenses_manage",

  // Custom pages admin ops
  cpCreate: "custom_admin",
  cpDelete: "custom_admin",
  cpEdit: "custom_admin",
  cpAddField: "custom_admin",
  cpDelField: "custom_admin",
  cdDelete: "custom_admin",

  // Roles
  createInvite: "roles_manage",
  revokeInvite: "roles_manage",
  saveUserRole: "roles_manage",

  // Employees (تأمين)
  newEmployee: "delete_master",
  editEmployee: "delete_master",
  toggleEmployee: "delete_master",
  deleteEmployee: "delete_master",
};

if (act && ACT_PERMS[act]) {
  const need = ACT_PERMS[act];
  if (!hasPerm(need)) {
    toast("غير مسموح حسب صلاحياتك", "warn");
    return;
  }
}
