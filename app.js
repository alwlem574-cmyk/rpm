/* نظام حسن الوليم RPM - V16 (Cloud only + Activity Log)
   - زباين + سيارات + سجل
...
async function addEmployeePayment(employeeId) {
  const e = await dbAPI.get("employees", employeeId);
  if (!e) return;

  const v = await formModal({
    title: `دفعة راتب • ${e.name}`,
    subtitle: "تسجل دفعة للموظف (بالدينار) ويمكن ربطها كمصروف رواتب",
    fields: [
      { name: "amount", label: "المبلغ", type: "number", cast: "number", required: true },
      { name: "date", label: "التاريخ", type: "date", default: ymdToday() },
      { name: "note", label: "ملاحظة", placeholder: "سلفة / راتب شهر..." },
      { name: "asExpense", label: "تسجيل كمصروف", type: "checkbox", default: true, help: "يضيفها أيضاً للمصروفات (رواتب)" },
    ],
  });
  if (!v) return;

  const whenTs = v.date ? new Date(v.date + "T00:00:00").getTime() : Date.now();
  const p = {
    id: "pay_" + uid().slice(3),
    employeeId,
  ...
  await dbAPI.put("payrollPayments", p, { note: `دفعة راتب: ${e.name}` });
  ...
  if (v.asExpense) {
    const exp = {
      ...
      amount: Number(v.amount || 0),
      category: "رواتب",
      note: `راتب: ${e.name}${p.note ? " • " + p.note : ""}`,
      createdAt: Date.now(),
    };
    await dbAPI.put("expenses", exp, { note: `مصروف رواتب: ${e.name}` });
  }
...
async function viewEmployeePayments(employeeId) {
  const e = await dbAPI.get("employees", employeeId);
  if (!e) return;

  const all = (await dbAPI.getAll("payrollPayments"))
    .filter(p => p.employeeId === employeeId)
    .sort((a,b)=> (Number(b.whenTs||b.createdAt||0) - Number(a.whenTs||a.createdAt||0)));

  const total = all.reduce((s,p)=> s + Number(p.amount||0), 0);

  const rows = all.slice(0, 60).map(p => `
    <tr class="tr">
      <td class="small">${fmtDay(p.whenTs||p.createdAt||0)}</td>
      <td><b>${money(p.amount||0)}</b></td>
      <td class="small">${escapeHtml(p.note||"—")}</td>
      <td><button class="btn btn-danger" data-act="delEmpPay" data-id="${p.id}">حذف</button></td>
    </tr>
  `).join("");

  // reuse print modal container
  ensureMiniModal();
  const mm = $("#miniModal");
  $("#mmTitle").textContent = `سجل دفعات • ${e.name}`;
  $("#mmBody").innerHTML = `
    <div class="card subcard">
      <div class="kv"><span>إجمالي الدفعات (كل الوقت)</span><b>${money(total)}</b></div>
      <div style="height:10px"></div>
      <button class="btn btn-primary" data-act="empPay" data-id="${e.id}">+ إضافة دفعة</button>
    </div>
    <div style="height:10px"></div>
    ${all.length ? `
      <table class="table">
    ...
    ` : `<div class="notice">ماكو دفعات مسجلة.</div>`}
  `;
  mm.classList.remove("hidden");
}

async function deleteEmployeePayment(payId) {
  if (!confirm("حذف الدفعة؟")) return;
  await dbAPI.del("payrollPayments", payId);
  toast("تم الحذف ✅");
  renderRoute();
}

async function setEmployeeRole(employeeId) {
  if (!cloudEnabled()) return toast("السحابة غير مفعلة.");
  if (currentRole() !== "admin") return toast("هذا الخيار للمدير فقط.");

  const e = await dbAPI.get("employees", employeeId);
  if (!e) return;

  if (!e.email) {
    toast("لازم تضيفي إيميل للموظف حتى نربطه بالصلاحيات.");
    return editEmployee(employeeId);
  }

  const v = await formModal({
    title: `تعيين دور • ${e.name}`,
    subtitle: "هذا يحدد الصفحات المسموحة للموظف بعد تسجيل الدخول",
    fields: [
      ...
      { name: "linkEmployee", label: "ربط بالحساب", type: "checkbox", default: true, help: "يربط الدور بهذا الموظف داخل النظام" },
    ],
  });
  if (!v) return;

  const email = normEmail(e.email||"");
  const existing = await cloudAPI.getAll("rbacUsers");
  const me = (existing||[]).find(u => normEmail(u.email||"") === email);

  const obj = me ? { ...me } : { id: "u_" + uid().slice(3), email };
  obj.role = v.role || "pending";
  if (v.linkEmployee) obj.employeeId = e.id;

  obj.updatedAt = Date.now();

  await cloudAPI.put("rbacUsers", obj, { note: `تعيين دور: ${email} -> ${obj.role}` });
  toast("تم تعيين الدور ✅");
  renderRoute();
}
...
/* ======================== Fluids (Liters Inventory) ======================== */
const FLUID_KIND_LABEL = {
  engine: "زيت محرك",
  gear: "زيت كير",
  coolant: "ماء/سائل راديتور",
};
...
async function createFluid() {
  const v = await formModal({
    ...
    fields: [
      { name: "name", label: "الاسم/الماركة", required: true, placeholder: "مثال: Mobil" },
      { name: "kind", label: "النوع", type: "select", options: [
        { value: "engine", label: FLUID_KIND_LABEL.engine },
        { value: "gear", label: FLUID_KIND_LABEL.gear },
        { value: "coolant", label: FLUID_KIND_LABEL.coolant },
      ], required: true },
      { name: "spec", label: "المواصفة/اللزوجة", placeholder: "مثال: 5W-30 / ATF Dexron VI / Red" },
      { name: "sku", label: "SKU / رقم" },
      { name: "buyPerL", label: "سعر الشراء / لتر", type: "number", cast: "number", step: 1 },
      { name: "sellPerL", label: "سعر البيع / لتر", type: "number", cast: "number", step: 1 },
      { name: "liters", label: "الرصيد الحالي (لتر)", type: "number", cast: "number", required: true, default: 0, step: 0.01 },
      { name: "minLiters", label: "الحد الأدنى (لتر)", type: "number", cast: "number", default: 0, step: 0.01 },
    ],
  });
  if (!v) return;

  const f = {
    id: "fluid_" + uid().slice(3),
    name: (v.name || "").trim(),
    kind: (v.kind || "engine").trim(),
    spec: (v.spec || "").trim(),
    sku: (v.sku || "").trim(),
    buyPerL: Number(v.buyPerL || 0),
    sellPerL: Number(v.sellPerL || 0),
    liters: Number(v.liters || 0),
    minLiters: Number(v.minLiters || 0),
    createdAt: Date.now(),
  };

  await dbAPI.put("fluids", f);
  await logFluidMove({ fluidId: f.id, deltaLiters: f.liters, reason: "توريد/إضافة أولية" });

  toast("تمت إضافة مادة السائل ✅");
  renderRoute();
}

async function adjustFluidLiters(fluidId, delta) {
  const f = await dbAPI.get("fluids", fluidId);
  if (!f) return;

  const v = await formModal({
    title: delta > 0 ? "إضافة (لتر)" : "صرف (لتر)",
    subtitle: `${FLUID_KIND_LABEL[f.kind] || "سائل"} • ${f.name || ""} ${f.spec ? "• " + f.spec : ""}`,
    fields: [
      { name: "liters", label: delta > 0 ? "لترات للإضافة" : "لترات للصرف", type: "number", cast: "number", required: true, default: 1, step: 0.01 },
      { name: "note", label: "ملاحظة (اختياري)", placeholder: "مثال: توريد من المورد / تصحيح" },
    ],
    submitText: "تطبيق",
  });
  if (!v) return;

  const amount = Number(v.liters || 0);
  if (!amount || amount <= 0) return;

  const next = Number(f.liters || 0) + (delta > 0 ? amount : -amount);
  if (next < 0) return toast("ما يصير الرصيد يصير سالب.", "bad");

  const before = Number(f.liters || 0);
  f.liters = round2(next);
  await dbAPI.put("fluids", f);

  await logFluidMove({
    fluidId: f.id,
    deltaLiters: delta > 0 ? amount : -amount,
    reason: (v.note || "").trim() || (delta > 0 ? "إضافة" : "صرف"),
    before,
    after: f.liters,
  });

  toast("تم تحديث رصيد السوائل ✅");
  renderRoute();
}
...
async function deleteFluid(fluidId) {
  const f = await dbAPI.get("fluids", fluidId);
  if (!f) return;
  if (!confirm(`حذف المادة؟\n${FLUID_KIND_LABEL[f.kind] || "سائل"} • ${f.name}`)) return;
  await dbAPI.del("fluids", fluidId);
  toast("تم الحذف ✅");
  renderRoute();
}

async function logFluidMove({ fluidId, deltaLiters, reason, before=null, after=null, linkType="", linkId="" }) {
  const now = Date.now();
  const m = {
    id: "fmov_" + uid().slice(3),
    fluidId,
    deltaLiters: Number(deltaLiters || 0),
    reason: (reason || "").trim(),
    before,
    after,
    linkType,
...
const SERVICE_DEFAULTS = {
  engine: { km: 5000, months: 3, label: "زيت محرك", nextOdoKey: "nextOilOdo", nextDateKey: "nextOilDate" },
  gear: { km: 20000, months: 12, label: "زيت كير", nextOdoKey: "nextGearOdo", nextDateKey: "nextGearDate" },
  coolant: { km: 40000, months: 12, label: "ماء/سائل راديتور", nextOdoKey: "nextCoolantOdo", nextDateKey: "nextCoolantDate" },
};
...
async function createOilChangeInvoice() {
  ...
  const intervalKm = Number($("#oilInterval").value || (SERVICE_DEFAULTS[serviceKind]?.km || 5000));
  const nextOdoIn = Number($("#oilNextOdo").value || 0);

  const intervalMonths = Number($("#oilIntervalMonths")?.value || (SERVICE_DEFAULTS[serviceKind]?.months || 0));
  const nextDateStr = ($("#oilNextDate")?.value || "").trim();
  ...
  const nextOdo = nextOdoIn || (currentOdo + intervalKm);
  const nextDateTs = nextDateStr ? (new Date(nextDateStr + "T00:00:00").getTime()) : (intervalMonths ? addMonthsTs(now, intervalMonths) : 0);

  const sdef = SERVICE_DEFAULTS[serviceKind] || SERVICE_DEFAULTS.engine;
  const complaint = `خدمة ${sdef.label}`; // مو بس "تبديل دهن"
  ...
  // Items (هسه أسعار يدوي — لاحقاً نربطها بمخزون السوائل باللتر)
  const items = [];
  if (oilPrice > 0) items.push({ name: `${sdef.label} ${oilType || ""}`.trim() || sdef.label, qty: 1, unit: oilPrice, total: oilPrice, kind:"oil" });
  if (filterPrice > 0) items.push({ name: "فلتر", qty: 1, unit: filterPrice, total: filterPrice, kind:"part" });
  ...
  // Update vehicle: odometer + next due per service kind
  const v = await dbAPI.get("vehicles", vehicleId);
  if (v) {
    v.odometer = currentOdo;

    // set per-kind next fields
    v[sdef.nextOdoKey] = nextOdo;
    if (nextDateTs) v[sdef.nextDateKey] = nextDateTs;

    // keep legacy for engine
    if (serviceKind === "engine") v.nextOilOdo = nextOdo;

    // last service meta (for quick repeat)
    const lastKey = serviceKind === "engine" ? "lastEngineService" : (serviceKind === "gear" ? "lastGearService" : "lastCoolantService");
    v[lastKey] = { at: now, odo: currentOdo, oilType: oilType || "", price: oilPrice || 0, filter: filterPrice || 0, labor: laborPrice || 0, intervalKm, intervalMonths };

    await dbAPI.put("vehicles", v);
  }

  alert("تم تسجيل الخدمة + إنشاء فاتورة ✅");
  location.hash = `#/invoices`;
}
...
async function viewDashboard() {
  ...
  // Service reminders (km + date) for engine/gear/coolant
  const nowTs = Date.now();
  const dueKmThreshold = 300;      // قريب
  const overdueKmGrace = -200;     // متأخر شوي
  const dueDaysThreshold = 14;     // قريب (أسبوعين)

  const mkReminder = (v, kind) => {
    const def = SERVICE_DEFAULTS[kind] || SERVICE_DEFAULTS.engine;
    const nextOdo = Number(v?.[def.nextOdoKey] || 0);
    const curOdo = Number(v?.odometer || 0);
    const diffKm = (nextOdo && curOdo) ? (nextOdo - curOdo) : null;

    const nextDateTs = Number(v?.[def.nextDateKey] || 0);
    const diffDays = nextDateTs ? daysBetween(nowTs, nextDateTs) : null;

    const byKm = (diffKm != null) && (diffKm <= dueKmThreshold) && (diffKm >= overdueKmGrace);
    const byDate = (diffDays != null) && (diffDays <= dueDaysThreshold) && (diffDays >= -30);

    if (!byKm && !byDate) return null;

    const status =
      (diffKm != null && diffKm < 0) || (diffDays != null && diffDays < 0) ? "overdue" :
      (diffKm != null && diffKm <= dueKmThreshold) || (diffDays != null && diffDays <= dueDaysThreshold) ? "due" : "ok";

    return {
      vehicleId: v.id,
      customerId: v.customerId,
      plate: v.plate,
      makeLine: [v.make,v.model,v.year].filter(Boolean).join(" "),
      kind,
      label: def.label,
      curOdo: curOdo || null,
      nextOdo: nextOdo || null,
      diffKm,
      nextDateTs: nextDateTs || null,
      diffDays,
      status,
    };
  };
  ...
  for (const v of vehicles) {
    for (const k of ["engine","gear","coolant"]) {
      const r = mkReminder(v, k);
      if (r) reminders.push(r);
    }
  }
  reminders.sort((a,b)=> {
    const ak = (a.status==="overdue") ? -100000 : (a.diffKm ?? 999999);
    const bk = (b.status==="overdue") ? -100000 : (b.diffKm ?? 999999);
    return ak - bk;
  });
  const remindersTop = reminders.slice(0, 8);
  ...
  return `
    ...
    <div class="row" style="margin-top:12px">
      ...
      <div class="col">
        <div class="card">
          <div class="section-title">تنبيهات صيانة السوائل</div>
          <div class="small">محرك / كير / راديتور — حسب (كم) و/أو (تاريخ)</div>
          <div class="hr"></div>
          ${remindersTop.length ? remindersTop.map(r => {
            const c = cMap.get(r.customerId);
            const pillTxt = (() => {
              const kmPart = (r.diffKm!=null) ? (r.diffKm<0 ? `متأخر ${Math.abs(r.diffKm)} كم` : `باقي ${r.diffKm} كم`) : "";
              const dayPart = (r.diffDays!=null) ? (r.diffDays<0 ? `متأخر ${Math.abs(r.diffDays)} يوم` : `باقي ${r.diffDays} يوم`) : "";
              const both = [kmPart, dayPart].filter(Boolean).join(" • ");
              return both || "قريب";
            })();
            const cls = r.status==="overdue" ? "parts" : "progress";
            return `
              <div class="card subcard" style="margin-bottom:10px">
                <div style="display:flex;justify-content:space-between;gap:10px">
                  <div>
                    <div style="font-weight:900">${escapeHtml(c?.name || "—")} • ${escapeHtml(r.plate || "—")}</div>
                    <div class="small">${escapeHtml(r.makeLine || "")}</div>
                    <div class="small">الخدمة: <b>${escapeHtml(r.label)}</b></div>
                    <div class="small">
                      ${r.curOdo!=null? `حالي: <b>${r.curOdo}</b>`:""}
                      ${r.nextOdo!=null? ` • جاي: <b>${r.nextOdo}</b>`:""}
                      ${r.nextDateTs? ` • تاريخ: <b>${fmtDay(r.nextDateTs)}</b>`:""}
                    </div>
                  </div>
                  <div class="pill ${cls}">${escapeHtml(pillTxt)}</div>
                </div>
                <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap">
                  <a class="btn" href="#/customer?id=${encodeURIComponent(r.customerId)}">سجل الزبون</a>
                  <a class="btn btn-primary" href="#/oil?customerId=${encodeURIComponent(r.customerId)}&vehicleId=${encodeURIComponent(r.vehicleId)}&kind=${encodeURIComponent(r.kind)}">فتح خدمة</a>
                </div>
              </div>
            `;
          }).join("") : `<div class="notice">ماكو تنبيهات قريبة حالياً ✅</div>`}
...
async function viewVehicleDetails(vehicleId, params) {
  ...
  const nextGear = Number(v.nextGearOdo || 0);
  const diffGear = (odo && nextGear) ? (nextGear - odo) : null;
  const nextCoolant = Number(v.nextCoolantOdo || 0);
  const diffCoolant = (odo && nextCoolant) ? (nextCoolant - odo) : null;

  const nowTs = Date.now();
  const dOil = v.nextOilDate ? daysBetween(nowTs, Number(v.nextOilDate)) : null;
  const dGear = v.nextGearDate ? daysBetween(nowTs, Number(v.nextGearDate)) : null;
  const dCoolant = v.nextCoolantDate ? daysBetween(nowTs, Number(v.nextCoolantDate)) : null;

  const svcLine = (label, nextOdoVal, diffKmVal, nextDateTs, diffDaysVal, kind) => {
    const parts = [];
    if (nextOdoVal) parts.push(`كم: <b>${nextOdoVal}</b> ${diffKmVal!=null? `(${diffKmVal<0? "متأخر":"باقي"} ${Math.abs(diffKmVal)} كم)`:""}`);
    if (nextDateTs) parts.push(`تاريخ: <b>${fmtDay(nextDateTs)}</b> ${diffDaysVal!=null? `(${diffDaysVal<0? "متأخر":"باقي"} ${Math.abs(diffDaysVal)} يوم)`:""}`);
    const right = parts.length ? parts.join(" • ") : "—";
    const pillCls = (diffKmVal!=null && diffKmVal<0) || (diffDaysVal!=null && diffDaysVal<0) ? "parts" : "progress";
    const pillTxt = (diffKmVal!=null && diffKmVal<0) || (diffDaysVal!=null && diffDaysVal<0) ? "متأخر" : "قريب";
    return `
      <div class="kv" style="align-items:flex-start">
        <span><b>${escapeHtml(label)}</b><div class="small">${right}</div></span>
        <div style="display:flex; gap:8px; flex-wrap:wrap; justify-content:flex-end">
          ${(pillTxt!=="قريب" && pillTxt!=="متأخر") ? "" : `<span class="pill ${pillCls}">${pillTxt}</span>`}
          <a class="btn btn-soft" href="#/oil?customerId=${encodeURIComponent(v.customerId||"")}&vehicleId=${encodeURIComponent(v.id)}&kind=${encodeURIComponent(kind)}">فتح</a>
        </div>
      </div>
    `;
  };
      ...
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap">
        <div>
          ...
          <div class="card subcard" style="margin-top:10px">
            <div class="section-title">تنبيهات الصيانة</div>
            <div class="small">حسب (كم) و/أو (تاريخ) — محرك/كير/راديتور</div>
            <div class="hr"></div>
            ${svcLine("زيت محرك", nextOil, diffOil, Number(v.nextOilDate||0), dOil, "engine")}
            <div style="height:8px"></div>
            ${svcLine("زيت كير", nextGear, diffGear, Number(v.nextGearDate||0), dGear, "gear")}
            <div style="height:8px"></div>
            ${svcLine("راديتور", nextCoolant, diffCoolant, Number(v.nextCoolantDate||0), dCoolant, "coolant")}
          </div>
        </div>
  ...
  return `
    <div class="card">
      ...
      <div class="grid3">
        ...
        <div>
          <label class="small">فترة الخدمة (كم)</label>
        ...
        <div>
          <label class="small">فترة الخدمة (أشهر)</label>
          <select id="oilIntervalMonths" class="input">
      ...
      <div class="grid3" style="margin-top:12px">
        ...
        <div>
          <label class="small">العداد الجاي (كم)</label>
          <input id="oilNextOdo" class="input" inputmode="numeric" placeholder="155000" />
        </div>
        <div>
          <label class="small">تاريخ الخدمة الجاية</label>
          <input id="oilNextDate" class="input" type="date" />
        </div>
      </div>
...
async function viewInventory() {
  ...
  const fluids = (await dbAPI.getAll("fluids")).sort((a,b)=> ((FLUID_KIND_LABEL[a.kind]||"").localeCompare(FLUID_KIND_LABEL[b.kind]||"", "ar") || (a.name||"").localeCompare(b.name||"", "ar")));
  const lowF = fluids.filter(f => Number(f.liters||0) <= Number(f.minLiters||0));
    ...
    <div class="card">
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          ...
          <button class="btn btn-soft" data-act="newFluid">+ مادة سائل</button>
        </div>
      </div>

      <div class="hr"></div>
      ${low.length ? `
      <div class="notice">⚠️ قطع تحت الحد الأدنى: ${low.length}</div>
      <div class="small" style="margin-top:6px">إجراءات سريعة للقطع الناقصة:</div>
      <div style="overflow:auto;margin-top:8px">
        <table class="table">
          ...
          <tbody>
            ${low.map(p => `
              <tr class="tr">
                <td><b>${escapeHtml(p.name)}</b><div class="small">${escapeHtml(p.sku || "—")}</div></td>
                <td><b>${p.stock ?? 0}</b></td>
                <td class="small">${p.min ?? 0}</td>
                <td>
                  <div style="display:flex;gap:8px;flex-wrap:wrap">
                    <button class="btn btn-primary" data-act="stockAdd" data-id="${p.id}">+ إضافة</button>
      ...
      ${lowF.length ? `
      <div class="notice" style="margin-top:10px">⚠️ سوائل تحت الحد الأدنى: ${lowF.length}</div>
      <div class="small" style="margin-top:6px">إجراءات سريعة للسوائل الناقصة (باللتر):</div>
      <div style="overflow:auto;margin-top:8px">
        <table class="table">
          <thead>
            <tr>
          ...
          <tbody>
            ${lowF.map(f => `
              <tr class="tr">
                <td>
                  <b>${escapeHtml(FLUID_KIND_LABEL[f.kind] || "سائل")} • ${escapeHtml(f.name)}</b>
                  <div class="small">${escapeHtml(f.spec || "—")} ${f.sku ? "• "+escapeHtml(f.sku):""}</div>
                </td>
                <td><b>${round2(f.liters ?? 0)}</b></td>
                <td class="small">${round2(f.minLiters ?? 0)}</td>
                <td>
                  <div style="display:flex;gap:8px;flex-wrap:wrap">
                    <button class="btn btn-primary" data-act="fluidAdd" data-id="${f.id}">+ إضافة</button>
...
async function viewEmployees() {
  ...
  const payments = await dbAPI.getAll("payrollPayments");
  const payMap = new Map(); // employeeId -> {lastTs, lastAmount, monthPaid}
  const now = new Date();
  ...
  for (const p of payments) {
    ...
    if (!eid) continue;
    const prev = payMap.get(eid) || { lastTs: 0, lastAmount: 0, monthPaid: 0 };
    const ts = Number(p.whenTs || p.createdAt || 0);
    const amt = Number(p.amount || 0);
    if (ts > prev.lastTs) { prev.lastTs = ts; prev.lastAmount = amt; }
    if (ts >= startMonth) prev.monthPaid += amt;
    payMap.set(eid, prev);
  }
  ...
  if (cloudEnabled()) {
    try {
      const users = await cloudAPI.getAll("rbacUsers");
      roleByEmail = new Map((users||[]).map(u => [normEmail(u.email||""), u.role||"pending"]));
    } catch (_) {}
  }

  const q = (state.empQuery || "").trim().toLowerCase();
  const showInactive = !!state.empShowInactive;

  const filtered = employees.filter(e => {
    if (!showInactive && !e.active) return false;
    if (!q) return true;
    const hay = `${e.name||""} ${e.phone||""} ${e.email||""} ${e.specialty||""}`.toLowerCase();
    ...
    <div class="card">
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap">
        <div>
          ...
          <div class="small">بحث + رواتب + ربط صلاحيات + سجل دفعات</div>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
      ...
      <div class="grid2">
        <div>
          <div class="small" style="margin:4px 2px">بحث</div>
          <input id="empQuery" class="input" placeholder="اسم / هاتف / إيميل / اختصاص" value="${escapeHtml(state.empQuery||"")}" />
        </div>
        <div>
          <div class="small" style="margin:4px 2px">عرض المتوقفين</div>
          <label class="small" style="display:flex;gap:8px;align-items:center">
            <input id="empShowInactive" type="checkbox" ${showInactive ? "checked" : ""} />
            <span>إظهار الموظفين المتوقفين</span>
          </label>
        </div>
      </div>
      ...
      <div class="card subcard">
        <div class="grid3">
          ...
          <div class="kv"><span>دفعات هذا الشهر (المسجلة)</span><b>${money(Array.from(payMap.values()).reduce((s,x)=>s+Number(x.monthPaid||0),0))}</b></div>
        </div>
      </div>
      ...
      <table class="table">
        ...
        <tbody>
          ${filtered.map(e => {
            const r = roleByEmail.get(normEmail(e.email||"")) || "—";
            const pay = payMap.get(e.id) || { lastTs: 0, lastAmount: 0 };
            const lastPay = pay.lastTs ? `${fmtDay(pay.lastTs)} • ${money(pay.lastAmount||0)}` : "—";
            const sal = `${escapeHtml(e.salaryType||"—")} • ${money(e.salaryAmount||0)}`;
            return `
              <tr class="tr">
                ...
                <td class="small">${r === "—" ? "—" : escapeHtml(roleLabel(r))}</td>
                <td class="small">${sal}</td>
                <td class="small">${lastPay}</td>
                <td>${e.active ? `<span class="pill done">فعّال</span>` : `<span class="pill off">متوقف</span>`}</td>
                <td>
                  <div style="display:flex;gap:8px;flex-wrap:wrap">
                    <button class="btn" data-act="empPay" data-id="${e.id}">+ دفعة</button>
                    <button class="btn" data-act="empPays" data-id="${e.id}">السجل</button>
                    ${cloudEnabled() && currentRole()==="admin" ? `<button class="btn" data-act="setEmpRole" data-id="${e.id}">الدور</button>` : ``}
      ...
      ` : `<div class="notice">ماكو نتائج.</div>`}
    </div>

    <script>
      // tiny inline listeners (safe)
      (function(){
        const q = document.getElementById("empQuery");
        const cb = document.getElementById("empShowInactive");
        if (q) q.addEventListener("input", () => { window.state.empQuery = q.value; window.renderRoute(); });
        if (cb) cb.addEventListener("change", () => { window.state.empShowInactive = cb.checked; window.renderRoute(); });
      })();
    </script>
  `;
}
...
async function renderRoute() {
  ...
  if (route === "employees") {
    const q = $("#empQuery");
    if (q) q.addEventListener("input", () => { state.empQuery = q.value; renderRoute(); });
    const cb = $("#empShowInactive");
    if (cb) cb.addEventListener("change", () => { state.empShowInactive = cb.checked; renderRoute(); });
  }
  ...
  // Oil/Fluids Service: auto-calc next (km/date) + per-kind defaults
  if (route === "oil") {
    const kindSel = $("#oilServiceKind");
    const cur = $("#oilCurrentOdo");
    const intervalKm = $("#oilInterval");
    const intervalMonths = $("#oilIntervalMonths");
    const nextKm = $("#oilNextOdo");
    const nextDate = $("#oilNextDate");

    const applyKindDefaults = (kind) => {
      const def = SERVICE_DEFAULTS[kind] || SERVICE_DEFAULTS.engine;
      if (intervalKm && (!intervalKm.value || intervalKm.value === "0")) intervalKm.value = String(def.km);
      if (intervalMonths && (!intervalMonths.value || intervalMonths.value === "0")) intervalMonths.value = String(def.months);
      // always keep sensible defaults when switching kind
      if (intervalKm) intervalKm.value = String(def.km);
      if (intervalMonths) intervalMonths.value = String(def.months);
      recalcNextKm();
      recalcNextDate();
    };
    ...
    const recalcNextKm = () => {
      ...
      const it = Number(intervalKm?.value || 0);
      if (nextKm && c > 0 && it > 0) nextKm.value = String(c + it);
    };

    const recalcNextDate = () => {
      const m = Number(intervalMonths?.value || 0);
      if (!nextDate) return;
      if (!m) return; // بدون
      nextDate.value = ymdFromTs(addMonthsTs(Date.now(), m));
    };

    cur?.addEventListener("input", recalcNextKm);
    intervalKm?.addEventListener("change", recalcNextKm);
    intervalMonths?.addEventListener("change", recalcNextDate);
    kindSel?.addEventListener("change", () => applyKindDefaults(kindSel.value || "engine"));

    // init from query params
    const { params: ps } = parseHash();
    const vId = ps.get("vehicleId");
    const kindQ = (ps.get("kind") || "engine").trim();
    if (kindSel) kindSel.value = ["engine","gear","coolant"].includes(kindQ) ? kindQ : "engine";
    applyKindDefaults(kindSel?.value || "engine");

    // prefill from vehicle (per kind)
    if (vId) {
      const v = await dbAPI.get("vehicles", vId);
      if (v?.odometer) cur.value = String(v.odometer);

      const k = kindSel?.value || "engine";
      const def = SERVICE_DEFAULTS[k] || SERVICE_DEFAULTS.engine;

      if (v?.[def.nextOdoKey]) nextKm.value = String(v[def.nextOdoKey]);
      if (v?.[def.nextDateKey]) nextDate.value = ymdFromTs(Number(v[def.nextDateKey]));

      if (v?.odometer && (!v?.[def.nextOdoKey])) recalcNextKm();
      if (!v?.[def.nextDateKey] && Number(intervalMonths?.value||0)>0) recalcNextDate();
    }
  }
}
...
document.addEventListener("click", async (e) => {
  ...
  if (act === "saveUserRole") return saveUserRole(id);
    ...
    if (act === "empPay") return addEmployeePayment(id);
  if (act === "empPays") return viewEmployeePayments(id);
  if (act === "delEmpPay") return deleteEmployeePayment(id);
  if (act === "setEmpRole") return setEmployeeRole(id);
