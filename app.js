...
const CLOUD_ROOT_ONLY_STORES = new Set([]); // في نظام shop كل شيء يكون داخل shops/{shopId}
...
function shopId() {
  ...
  const existing = Settings.get("shopId", null);
  if (!u) return existing;
  if (existing) return existing;
  const sid = `shop_${u.uid}`;
  Settings.set("shopId", sid);
  return sid;
}
...
// Migration: Root => shops/{shopId}/...
if (act === "migrateToShop") {
  if (!confirm("سيتم نسخ البيانات من Root إلى shops/{shopId}. هل تريدين المتابعة؟")) return;
  if (!authState.user) { toast("سجّلي دخول أولاً", "warn"); return; }
  if (!hasPerm("roles_manage")) { toast("مدير فقط", "warn"); return; }

  const sid = shopId();
  toast("بدء الترحيل... لا تغلقي الصفحة", "info");

  // 1) أنشئ سجل الكراج + عضوية المالك
  try {
    const shopRef = doc(firestore, `shops/${sid}`);
    const shopSnap = await getDoc(shopRef);
    if (!shopSnap.exists()) {
      await setDoc(shopRef, { id: sid, createdAt: Date.now(), ownerUid: authState.user.uid, ownerEmail: authState.user.email||"", name: "Garage", version: 1 }, { merge: true });
    }
    const memRef = doc(firestore, `shops/${sid}/members/${authState.user.uid}`);
    await setDoc(memRef, { uid: authState.user.uid, email: authState.user.email||"", role: "admin", active: true, createdAt: Date.now() }, { merge: true });
  } catch (e) {
    console.error(e);
    toast("فشل إنشاء الكراج/العضوية. تأكدي من Rules", "err");
    return;
  }

  // 2) انسخ Collections الأساسية
  const COLS = ["cars","orders","parts","customers","employees","invoices","appointments","expenses","fluids","activity","attachments","settings"];
  let ok = 0, failedCols = [];
  ...
  for (const c of COLS) {
    try {
      ...
      let n = 0;
      snap.forEach(d => {
        batch.set(doc(firestore, `shops/${sid}/${c}/${d.id}`), d.data(), { merge: true });
        n++;
      });
      await batch.commit();
      ok += n;
    } catch (e) {
      console.warn("failed col", c, e);
      failedCols.push(c);
    }
  }

  // 3) فعّل وضع shop
  Settings.set("cloudScope", "shop");
  Settings.set("shopId", sid);

  toast(`تم الترحيل: ${ok} مستند. فشل: ${failedCols.length ? failedCols.join(", ") : "0"}.`, failedCols.length ? "warn" : "ok");
  try { await refreshEmployeesCache(); } catch {}
  renderRoute();
  return;
}
