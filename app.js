...
function employeeNameById(id) {
  try {
    if (!id) return "";
    const list = state._employeesCache || [];
    const e = list.find(x => x.id === id);
    return e ? (e.name || e.fullName || e.title || "") : "";
  } catch { return ""; }
}

async function refreshEmployeesCache() {
  try {
    state._employeesCache = await dbAPI.getAll("employees");
  } catch { state._employeesCache = []; }
}
...
<div class="grid2">
  ...
  <div>
    <div class="small" style="margin:4px 2px">الموظف</div>
    <input id="actEmp" class="input" placeholder="ID أو اسم" />
  </div>
...
async function viewActivity(params) {
  ...
  const roleQ = (params.get("role") || "").trim();
  const empQ = (params.get("emp") || "").trim();
  ...
  return `
    <div class="card">
      ...
      <div class="row" style="align-items:center; gap:10px; flex-wrap:wrap">
        ...
        ${state.employeeId ? `<span class="badge">الموظف: ${escapeHtml(employeeNameById(state.employeeId) || state.employeeId)}</span>` : ``}
      </div>
